document.getElementById("loginBtn").addEventListener("click", async function () {
    ...
    try {
        ...
        const response = await fetch("https://api.sarkhanrahimli.dev/api/filmalisa/auth/signup", {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
            ...
            messageDiv.textContent = data.message || "Qeydiyyat zamanı xəta baş verdi.";
            messageDiv.style.color = "red";
        }
    } catch (error) {
        console.error("Network error:", error);
        messageDiv.textContent = "Şəbəkə xətası baş verdi. Zəhmət olmasa yenidən yoxlayın.";
        messageDiv.style.color = "red";
    }
});
